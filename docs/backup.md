...
O `CACHE_NAME` do Service Worker é versionado (atualmente `devedor-pwa-v7`); ao alterar `index.html`, `backup.js` ou `manifest.json`, incremente a versão para que usuários recebam a atualização.